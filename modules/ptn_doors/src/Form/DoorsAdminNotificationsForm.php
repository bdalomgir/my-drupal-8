...
// 'Drupal\ptn_doors\Form\DoorsAdminNotificationsForm::adminNotificationFormAJAX',
    $form['electric_add'] = array(
      '#type' => 'submit',
      '#value' => t('Add Electric Door Contact'),
      '#ajax' => array(
        'callback' => array($this, 'adminNotificationFormAJAX'),
   ...
   public function adminNotificationFormAJAX(array &$form, FormStateInterface $form_state){
     ...
     // validate
     $wrapper = $form_state->getTriggeringElement();
     $wrapper = $wrapper['#ajax']['wrapper'];
     $wrapper_ID = $wrapper;
     ...
     $new_contact = $form_state->getUserInput();
     $new_contact = $new_contact[$contact_field];
     ...
     //echo $var_name."-|-".$new_list."1";
     \Drupal::state('ptn_doors.settings')->set($var_name, $new_list);
//     \Drupal::service('ptn_doors.settings')->set($var_name, $new_list)->save();
