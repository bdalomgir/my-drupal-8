<?php
 ...
 class DoorsAdminImportForm extends FormBase {
   ...
   public function submitForm(array &$form, FormStateInterface $form_state) {
     ...
     foreach(array('building_import','door_import') as $file_name){
       ...
       */
       ...
       if ($file) {
         ...
         if($file){
           if($file_name == "building_import"){
             $building_file = $file;
             $building_file_id = $file_data[0];
           } else if($file_name == "door_import"){
             $door_file = $file;
             $door_file_id = $file_data[0];
           }
           //$form_state->set(['storage', $file_name], $file);
         } else {
           drupal_set_message($this->t('The file %file_name failed to save to public', array('file_name' => $file_name)), 'error');
    ...
    //var_dump($building_file);
    //$building_file_path = $building_file->getFileUri();
    //exit(drupal_realpath($building_file_path));
    $building_csv = array_map('str_getcsv', file(drupal_realpath($building_file->getFileUri())));
    $is_buildings_valid = $this->importBuildingsValidation($building_csv);
    if($is_buildings_valid[0] === true){
      $door_csv = array_map('str_getcsv', file(drupal_realpath($door_file->getFileUri())));
      ...
      if($is_doors_valid[0] === true){
        ...
        file_delete($door_file_id);
        file_delete($building_file_id);
       ...
       foreach ($building_data as $building) {
         try {
           db_insert('ptn_buildings')
             ->fields(array(
               'bid' => abs($bid),
               'building_name' => $this->t($building[0]),
               'elec_or_man' => $this->t($building[1]),
             ))
             ->execute();
         } catch (PDOException $e) {
           drupal_set_message($this->t('Error: %message', array('%message' => $e->getMessage())), 'error');
         }
       }
     }
   }
